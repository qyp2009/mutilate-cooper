...
public:
  ...
  uint64_t total() {
    ...
    //for(vector<uint64_t>::iterator it  = bins.begin(); it != bins.end(); it++){
    //    sum += *it;
    //}
  ...
  void accumulate(const LogHistogramSampler &h) {
    ...
    //for(vector<Operation>::iterator it  = h.samples.begin(); it != h.samples.end();it++){
    //for(int i=0;i<h.samples.size();i++){
